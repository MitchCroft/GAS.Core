import { IEncodingProvider } from "../Encoding";
import { IEncryptionProvider } from "./Interfaces";
import { IInjectionTarget, IDependencyResolver } from "../DI/Interfaces";

/**
 * Handle basic encryption processes for string values that are provided
 */
export class BasicEncryptionProvider implements IEncryptionProvider, IInjectionTarget {
    /*----------Variables----------*/
    //PRIVATE
     ...
     */
    private _encodingProvider: IEncodingProvider | null;
    ...
    public construct(resolver: IDependencyResolver): void {
        this._encodingProvider = resolver.resolve("IEncodingProvider") as IEncodingProvider;
    }
    ...
    public encrypt(plainText: string, password: string): string {
        // If we don't have the encoding provider, this is going to be tricky
        if (this._encodingProvider === null) {
            throw `NullReferenceException: The IEncodingProvider property hasn't been set`;
        }
    ...
    private stringToBytes(str: string): number[] {
        ...
        for (let i = 0; i < str.length; ++i) {
            ...
            if (codePoint === undefined) {
                throw `InvalidFormatException: Received code point for the string '${str}' at index ${i} us undefined`;
            }
